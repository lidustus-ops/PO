...
string load_file_content(const string& fileName) {
    ...
    if (!targetFile.is_open()) {
        cout << "ERROR: Cannot find file at: " << fullPath << endl;
        return "";
    }
...
int main() {
    ...
    if (bind(listener, (struct sockaddr*)&service, sizeof(service)) == SOCKET_ERROR) {
    ...
    while (true) {
        ...
        if (session != INVALID_SOCKET) {
            thread worker(process_connection, session);
            worker.detach();
        }
    }
